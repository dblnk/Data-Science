...
p2 <- temp %>%
  ...
  geom_text(aes(x=40, y=3400, label=paste0("Median = ",signif(median(n),digits=3))), size=3, color="blue")+
...
Movie ratings show a slightly skewed, right-sided distribution, with `r percent (mean(temp$avg_rating >= global_mean), accuracy=0.1)` of all movie ratings scoring above overall average rating (Fig. 1, left). It is also obvious that 50% of the movies received less than `r median(temp$n)` and 25% received less than `r quantile(temp$n)[2]` ratings (Fig. 1, right). Estimating the average movie rating from the opinion of only a few users might not be very robust. Regularization was applied to introduce a penalty for low amount of ratings.
We started building our model by accounting for the movie effect $m_{i}$ for movie $i$:
\begin{center} $\bar{R_{i}} = \hat\mu + \hat{m}_{i}$, with average movie rating $\bar{R_{i}}$ and global mean estimate $\hat\mu$. \end{center}
...
\begin{center} $ \hat{u}_{j}= \frac{1}{I_{j}}\sum_{i=1}^{I_{j}}R_{i,j} - \hat{\mu} - \hat{m}_{i} $, with $I_{j}$ being the number of movies user $j$ has rated.
\end{center}  
...
df <- data.frame(userxgenre_rmses=c(0.8523471, 0.8506877, 0.8503473, 0.8503779, 0.8505495, 0.8507806, 0.8510362, 0.8512996, 0.8515626, 0.8518207, 0.8520716), l=seq(0,100,10))

par(mar=c(3,3,0.5,0.5))
plot(df$l, df$userxgenre_rmses, xlab= "lambda", ylab="RMSE", mgp=c(1.7,0.5,0))
